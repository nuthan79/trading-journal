...
export function equityCurve(closed, { openingCapital = 0, flows = [] } = {}) {
  ...
  const rows = chronological(closed).flatMap((t) =>
    ...
    bankedEvents(t).map((e) => ({
      ...
      interest: e.interest || 0, fees: e.fees || 0,
      marginCounted: e.marginCounted !== false,
    }))
  ).sort((a, b) => {
  ...
  return {
    ...
    interest: rows.reduce((s, t) => s + (Number(t.interest) || 0), 0),
    fees: rows.reduce((s, t) => s + (Number(t.fees) || 0), 0),
    /* False when any MTF in here was only reported, not taken out — the
       wording on every Net P&L depends on it. */
    marginCounted: !rows.some((t) => Number(t.margin) > 0 && t.marginCounted === false),
...
export function byPeriod(
  ...
  const put = (k, when, t, pnl, r, charge, margin = 0, interest = 0, marginCounted = true) => {
    if (!buckets.has(k)) {
    ...
    b.events.push({ trade: t, when, pnl, r, charge, margin, interest, marginCounted });
  };
  ...
  for (const t of rows) {
    if (byEntry) {
      ...
      put(label(t.entry_date), t.entry_date, t, p, rr, n(t.charges) || 0,
          n(t.realisedMargin) || 0, n(t.realisedInterest) || 0, t.marginInPnl !== false);
      continue;
    }
    ...
    for (const e of events) {
      put(label(e.date), e.date, t, e.pnl, e.r, e.charge, e.margin, e.interest, e.marginCounted !== false);
    }
  }
  ...
  for (const b of [...buckets.values()].sort((a, b) => new Date(a.first) - new Date(b.first))) {
    ...
    const interest = b.events.reduce((a, e) => a + (isFinite(e.interest) ? e.interest : 0), 0);
    const marginCounted = !b.events.some((e) => e.margin > 0 && e.marginCounted === false);
...
export function headline(closed, { openingCapital = 0, flows = [], banking = null } = {}) {
  ...
  return {
    ...
    fees: eq.fees,
    marginCounted: eq.marginCounted,

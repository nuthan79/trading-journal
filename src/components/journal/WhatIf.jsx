...
/**
 * What-if: the expectancy calculator, filled in from trades that actually
 * happened.
 *
 ...
 *
 * WHY IT IS CALLED WHAT-IF AND NOT EDGE. It held the name Edge for a day, which
 * was wrong twice over: `edge.js` computes the dimension breakdown next door,
 * so the modules and the tabs disagreed — and more to the point, this screen is
 * a question rather than a finding. It starts from measured numbers and then
 * invites you to move them, which is the opposite of the table beside it that
 * reports what actually happened and will not be dragged.
 */
export default function WhatIf({ closed = [], accountSize, defaultRiskPct }) {
  const p = useMemo(
